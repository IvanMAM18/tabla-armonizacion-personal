...
import ModalCarpetaPublica from './ModalCarpetaPublica';
import axios from 'axios';

export default function TablaTitulosPublica () {
    ...
    return (
        <div className="y-4">
            <div className='relative mt-2 mx-auto overflow-x-auto'>
                        ...
                        {currentData.map((record, index) => (
                            <tr key={index} className={`${record.tipo === 'titulo' ? 'bg-orange-200' : ''}`}>
                                ...
                                <td className='border border-y-slate-600 text-center'>
                                    <svg 
                                        onClick={() => handleOpenModal(record.id, 'ARCHIVOS', record.nombre)} 
                                ...
                                <td className='border border-y-slate-600 text-center'>
                                    <svg 
                                        onClick={() => handleOpenModal(record.id, 'FORMATOS', record.nombre)} 
                                        className="h-5 w-5 mx-auto text-emerald-900 cursor-pointer" 
                                        width="24" height="24" 
                                        viewBox="0 0 24 24" 
                                        strokeWidth="2" 
                ...
                <ModalCarpetaPublica isOpen={isModalOpen} onClose={handleCloseModal} id={modalId} tipo={modalTipo} nombre={modalNombre}/>
